...
class BinaryMemoryMapDataset(Dataset):
    ...
    def __init__(self, file_path, block_size, dtype=np.uint16):
        ...
        # We do NOT open the memory-mapped file here in __init__. We open
        # it lazily, the first time __getitem__ is called. This matters
        # because PyTorch's DataLoader can spawn separate worker
        # processes, and each process needs its own independent file
        # handle -- opening it too early can cause problems when workers
        # are forked.
    ...
    def __getitem__(self, index):
        # Open the memory-mapped file on first use (see note above).
        ...
        # position.
        raw_slice = self.memory_map[start_position:end_position]
...
def main():
    ...
    train_dataset = BinaryMemoryMapDataset(train_bin_path, args.block_size)
    val_dataset = BinaryMemoryMapDataset(val_bin_path, args.block_size)
